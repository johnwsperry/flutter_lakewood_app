...
class Databases {
  ...
  //Get Databases
  static Future<List<Database>> getAllHouseDatabasesFromName(
    String name,
  ) async {
    List<Database> returnList = [];
    Iterable<HouseDatabaseLocation>? matchingData =
        (await queryDatabaseLocationTable()).getAllDataByName(name);
  ...
  static Future<Database> getHouseDatabaseFromId(int id) async {
    HouseDatabaseLocationTable houseDatabases =
        await queryDatabaseLocationTable();
    //Get the database
  ...
  static Future<HouseDatabaseLocationTable> queryDatabaseLocationTable() async {
    ...
    if (_houseDatabases == null) {
      ...
      Database housesDatabase = await _openDatabase(
        global_vars.homesName,
        global_vars.houseDbLocationDataCreation,
      );
      //Save it and close it
      final List<Map<String, Object?>> databases = await housesDatabase.query(
        'HouseDatabase',
      );
  ...
  static Future<void> cloneHouse(String path) async {
    ...
    bool fileExists =
        FileSystemEntity.typeSync(databasePath) !=
  ...
  static Future<List<int>> getBytesFromFile(File file) async {
    ...
    Uint8List byteDataBytes = byteData.buffer.asUint8List(
